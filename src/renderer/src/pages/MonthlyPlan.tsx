...
const TYPE_BADGE: Record<GoalItem['type'], string> = {
  business: 'font-mono text-[10px] px-2 py-0.5 rounded bg-blue-500/10 text-blue-400 border border-blue-500/20',
  personal:
    ...
    async function loadMonthPlan(): Promise<void> {
      try {
        ...
        const subgoalsByGoal = await Promise.all(
          fetchedGoals.map((goal) => window.api.subgoals.getByGoal(goal.id) as Promise<SubgoalItem[]>),
        )
    ...
    <div className="h-full w-full overflow-y-auto bg-[var(--bg-base)]">
      <div className="max-w-4xl mx-auto px-8 py-8">
        <div className="flex justify-between items-start mb-6">
          <div>
            <p className="font-mono text-xs tracking-widest text-[var(--text-muted)] uppercase mb-1">PLAN</p>
        ...
        {goals.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20">
            ...
            <p className="text-sm text-[var(--text-secondary)] mb-3">No goals set for this month.</p>
            <button
                ...
                >
                  <div className="px-5 py-4 flex items-center justify-between border-b border-[var(--border-subtle)]">
                    <div className="flex items-center">
                      ...
                      <p className="text-sm font-medium text-[var(--text-primary)] ml-3">{goal.title}</p>
                    </div>
                    ...
                    {subgoals.length > 0 ? (
                      subgoals.map((subgoal) => (
                        <div key={subgoal.id} className="flex items-center gap-3 py-1.5">
                          ...
                          <span className={PRIORITY_BADGE[subgoal.priority]}>{subgoal.priority}</span>
                        </div>
                      ))
                    ) : (
